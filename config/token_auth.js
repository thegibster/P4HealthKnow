...
// Defines the JWTs contents, or payload, given a patient object. Make
...
/**
* Creates a JWT and returns it to the patient in the response.
...
* 2.  Status 403 Forbidden (when the patient can't be find or password fails)
...
function create(req, res, next) {
 ...
 Patient
   ...
   .then(function(patient) {
     if (!patient || !patient.verifyPasswordSync(req.body.password)) {
       var message = 'Patient not found or password incorrect.';
...
/**
* Creates a new JWT for an authenticated patient (using the authenticate
...
function generateJwt(patient, options) {
 return jwt.sign(
   extractPayload(patient, options),
